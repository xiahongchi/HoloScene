...
from MVMeshRecon.MeshRecon.project_mesh import multiview_color_projection, get_cameras_list, get_cameras_list_py3d
...
def simple_clean_mesh(pyml_mesh: ml.Mesh, apply_smooth=True, stepsmoothnum=1, apply_sub_divide=False, sub_divide_threshold=0.25, target_faces=25000):
    ms = ml.MeshSet()
    ...
    for i in tqdm(range(steps)):
        ...
        if i < update_warmup or i % update_normal_interval == 0:
            with torch.no_grad():
                ...
                _, _, target_normal = from_py3d_mesh(multiview_color_projection(
                    py3d_mesh, target_images, cameras_list=cameras, weights=None, confidence_threshold=0.2,
                    complete_unseen=False, below_confidence_strategy='original', reweight_with_cosangle='linear', opt_render = renderer))
