...
if "__main__" == __name__:
    ...
    with torch.no_grad():
        ...
        for rgb_path in tqdm(rgb_filename_list, desc="Estimating depth", leave=True):
            ...
            # I want to add image cropping if the size is too large and we infer normal.
            input_image_np = np.array(input_image, dtype=np.uint8)
            if False and normals and max(input_image_np.shape[0], input_image_np.shape[1]) > processing_res:
                image_slice = 2
                h, w = input_image_np.shape[:2]
                hs = np.linspace(0, int(0.4 * h), image_slice).astype(np.int32)
                he = np.linspace(int(0.6 * h), h, image_slice).astype(np.int32)
                ws = np.linspace(0, int(0.4 * w), image_slice).astype(np.int32)
                we = np.linspace(int(0.6 * w), w, image_slice).astype(np.int32)

                weights = np.zeros((h, w))
                normals_pred = np.zeros((3, h, w))
                normals_colored = np.zeros((h, w, 3))

                for slice_i_h in range(image_slice):
                    for slice_i_w in range(image_slice):
                        input_image_slice = Image.fromarray(input_image_np[hs[slice_i_h]:he[slice_i_h], ws[slice_i_w]:we[slice_i_w]])
                        ...
                        weights[hs[slice_i_h]:he[slice_i_h], ws[slice_i_w]:we[slice_i_w]] += 1.0
                        normals_pred[:, hs[slice_i_h]:he[slice_i_h], ws[slice_i_w]:we[slice_i_w]] += pred
                        normals_colored[hs[slice_i_h]:he[slice_i_h], ws[slice_i_w]:we[slice_i_w]] += np.array(pred_colored, dtype=np.float32) / 255.0
                ...
                weights += image_slice * image_slice
                normals_pred += pred * image_slice * image_slice
                normals_colored += np.array(pred_colored, dtype=np.float32) / 255.0 * image_slice * image_slice
                


                normals_pred = normals_pred / weights.reshape(1, h, w)
                normals_colored = normals_colored / weights.reshape(h, w, 1)

                pred = normals_pred
                pred_colored = Image.fromarray(np.clip(normals_colored * 255.0, 0, 255).astype(np.uint8))
            ...
            if normals:
                normal_im = np.array(pred_colored, dtype=np.float32) / 255.
                normal_im = 1.0 - normal_im
                normal_im = Image.fromarray(np.clip(normal_im * 255.0, 0, 255).astype(np.uint8))
                normal_im.save(os.path.join(output_dir, f"{rgb_name_base}.png"))
            else:
                np.save(os.path.join(output_dir, f"{rgb_name_base}.npy"), pred)
