...
def get_camera_params(uv, pose, intrinsics, ray_offset=None):
    ...
    if ray_offset is not None:
        x_cam += ray_offset[:, :, 0].reshape(batch_size, -1)
        y_cam += ray_offset[:, :, 1].reshape(batch_size, -1)

    z_cam = depth.view(batch_size, -1)
    # print("x_cam", x_cam.shape, x_cam)
    # print("y_cam", y_cam.shape, y_cam)
    # print("z_cam", z_cam.shape, z_cam)

    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics)
    # print("1 pixel_points_cam", pixel_points_cam.shape, pixel_points_cam)

    # permute for batch matrix product
    pixel_points_cam = pixel_points_cam.permute(0, 2, 1)
    # print("2 pixel_points_cam", pixel_points_cam.shape, pixel_points_cam)

    # import pdb; pdb.set_trace();
    world_coords = torch.bmm(p, pixel_points_cam).permute(0, 2, 1)
    # print("1 world_coords", world_coords.shape, world_coords)
    world_coords = world_coords[..., :3] / world_coords[..., 3:4]
    # print("2 world_coords", world_coords.shape, world_coords)
    # assert False
