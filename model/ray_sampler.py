...
class UniformSampler(RaySampler):
    ...
    def get_z_vals_near_far(self, ray_dirs, cam_loc, model, near, far):

        near, far = near * torch.ones(ray_dirs.shape[0], 1).cuda(), far * torch.ones(ray_dirs.shape[0], 1).cuda()
...
class ErrorBoundSampler(RaySampler):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                if idx==None:
                    ...
                    samples_sdf = model.implicit_network.get_multi_object_sdf_vals(points_flat, idx)
    ...
    def get_z_vals_near_far(self, ray_dirs, cam_loc, model, near, far, idx=None):
        beta0 = model.density.get_beta().detach()

        # Start with uniform sampling
        z_vals, near, far = self.uniform_sampler.get_z_vals_near_far(ray_dirs, cam_loc, model, near, far)
        samples, samples_idx = z_vals, None
