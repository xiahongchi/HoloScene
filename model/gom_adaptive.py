...
def compose_for_export_pt(model: GoM, select_idxs=None):
    if select_idxs is None:
        means = model.means().detach().clone().cpu()
        shs_0 = model.shs_0().detach().clone().cpu()
        shs_rest = model.shs_rest().detach().clone().cpu()
        colors = model.colors().detach().clone().cpu()
        opacities = model.opacities().detach().clone().cpu()
        scales = model.scales().detach().clone().cpu()
        quats = model.quats().detach().clone().cpu()
    else:
        means = model.means()[select_idxs].detach().clone().cpu()
        shs_0 = model.shs_0()[select_idxs].detach().clone().cpu()
        shs_rest = model.shs_rest()[select_idxs].detach().clone().cpu()
        colors = model.colors()[select_idxs].detach().clone().cpu()
        opacities = model.opacities()[select_idxs].detach().clone().cpu()
        scales = model.scales()[select_idxs].detach().clone().cpu()
        quats = model.quats()[select_idxs].detach().clone().cpu()
...
def compose_for_export(model: GoM, select_idxs=None):
    if select_idxs is None:
        ...
        means_drawer_i = model.means()[select_idxs].detach().clone().cpu().numpy()
        shs_0_drawer_i = model.shs_0()[select_idxs].detach().clone().contiguous().cpu().numpy()
        shs_rest_drawer_i = model.shs_rest()[select_idxs].detach().clone().transpose(1, 2).cpu().numpy()
        colors_drawer_i = torch.clamp(model.colors()[select_idxs].detach().clone(), 0.0, 1.0).data.cpu().numpy()
        opacities_drawer_i = model.opacities()[select_idxs].detach().clone().cpu().numpy()
        scales_drawer_i = model.scales()[select_idxs].detach().clone().cpu().numpy()
        quats_drawer_i = model.quats()[select_idxs].detach().clone().cpu().numpy()
...
def load_3D_points_on_mesh(texture_mesh_path, area_to_subdivide = 2e-6):
    ...
    if ext == ".obj":
        ...
        mesh_vert_normals = mesh.verts_normals_packed().clone().reshape(-1, 3)
        mesh_verts = mesh_verts - mesh_vert_normals * 0.00
        ...
        mesh_vert_normals = torch.from_numpy(mesh.vertex_normals).float()
        mesh_verts = mesh_verts - mesh_vert_normals * 0.00
        N_Gaussians = mesh_faces.shape[0]
        triangles = mesh_verts[mesh_faces.reshape(-1)].reshape(-1, 3, 3)
        means = torch.mean(triangles, dim=1)
...
def concatenate_seed_mesh(seed_mesh_list):
    ...
    for seed_mesh in seed_mesh_list:
        means_list.append(seed_mesh["means"])
        radius_list.append(seed_mesh["radius"])
        features_dc_list.append(seed_mesh["features_dc"])
        normals_list.append(seed_mesh["normals"])

        mesh_verts_list.append(seed_mesh["mesh_verts"])
        mesh_faces_list.append(seed_mesh["mesh_faces"] + num_mesh_verts)

        num_mesh_verts += seed_mesh["mesh_verts"].shape[0]
        num_faces_list.append(seed_mesh["mesh_faces"].shape[0]+num_faces_list[-1])
        instance_face_indices.append(torch.arange(num_faces_list[-2], num_faces_list[-1]).reshape(-1).long())
    ...
    mesh_verts = torch.cat(mesh_verts_list, dim=0)
    mesh_faces = torch.cat(mesh_faces_list, dim=0)

    seed_mesh_concatenated = {
        "means": means,
        "radius": radius,
        "features_dc": features_dc,
        "normals": normals,
    ...
    return seed_mesh_concatenated, instance_face_indices
...
class GoM(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(seed_mesh, list):
            seed_mesh_list = []
            for mesh in seed_mesh:
                seed_mesh_list.append(load_3D_points_on_mesh(mesh, area_to_subdivide))
            self.seed_mesh, self.instance_gs_idxs = concatenate_seed_mesh(seed_mesh_list)
        else:
            self.seed_mesh = load_3D_points_on_mesh(seed_mesh, area_to_subdivide)
    ...
    def export_gs(self, export_path, select_idxs=None):
        map_to_tensors, count = compose_for_export(self, select_idxs)
        write_ply(export_path, count, map_to_tensors)

    def export_gs_pt(self, export_path, select_idxs=None):
        export_dict_pt = compose_for_export_pt(self, select_idxs)
        torch.save(export_dict_pt, export_path)
    ...
    def opacities(self):
        if hasattr(self, 'visible_gs_indices') and self.visible_gs_indices is not None:
    ...
    def remove_from_optim(self, optimizer, group_i, deleted_mask, new_params):
        """removes the deleted_mask from the optimizer provided"""
        assert len(new_params) == 1
        # assert isinstance(optimizer, torch.optim.Adam), "Only works with Adam"

        param = optimizer.param_groups[group_i]["params"][0]
        ...
        del optimizer.param_groups[group_i]["params"][0]
        del optimizer.param_groups[group_i]["params"]
        optimizer.param_groups[group_i]["params"] = new_params
        optimizer.state[new_params[0]] = param_state

    def remove_from_all_optim(self, optimizer, deleted_mask):
        ...
        for group_i, group in enumerate(optimizer.param_groups):
            ...
            assert param_name in param_groups, f"Parameter group {param_name} not found in model parameters."

            self.remove_from_optim(optimizer, group_i, deleted_mask, param_groups[param_name])

        torch.cuda.empty_cache()

    def dup_in_optim(self, optimizer, group_i, dup_mask, new_params, n=2):
    ...
    def dup_in_all_optim(self, optimizer, dup_mask, n):
        param_groups = self.get_gaussian_param_groups()
        for group_i, group in enumerate(optimizer.param_groups):
            param_name = group['name']
            assert param_name in param_groups, f"Parameter group {param_name} not found in model parameters."
            self.dup_in_optim(optimizer, group_i, dup_mask, param_groups[param_name], n)
    ...
    def refinement_after(self, optimizer, step):
        assert step == self.step
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                self.dup_in_all_optim(optimizer, split_idcs, nsamps)

                dup_idcs = torch.where(dups)[0]
                self.dup_in_all_optim(optimizer, dup_idcs, 1)
            ...
            if deleted_mask is not None:
                self.remove_from_all_optim(optimizer, deleted_mask)
    ...
    def get_outputs(self, pose, K, H, W, camera_model) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        self.radii = torch.norm(info["radii"][0].float(), dim=-1).reshape(-1).int()  # [N]
        alpha = alpha[:, ...]
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        acm_gt = batch["acm"] # H, W, 1
        loss_acm = torch.mean(torch.abs(accumulation - acm_gt.reshape(accumulation.shape))) * self.config.acm_lambda
        ...
        if "mesh_depth" in batch:
            mesh_depth = batch["mesh_depth"] # H, W, 1
        ...
        loss_dict = {
            "main_loss": ((1 - self.config.ssim_lambda) * Ll1 + self.config.ssim_lambda * simloss) * self.config.rgb_lambda + loss_acm + L1_depth,
            # "main_loss": Ll1,
            "scale_reg": scale_reg,
        }
